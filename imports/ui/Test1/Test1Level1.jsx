...
var Test1Attempts = 0;
var Test1TotalCorrect = 0;
var Test1TotalWrong = 0;
var a = 2;
...
export default class Test1Level1 extends Component {
	constructor(props) {
		super(props);
		this.state = {
			firstSlider:0,
	...
	onSubmit(e) {
		...
		var x = document.getElementById('Level1Slider1');
		var y = document.getElementById('Level1Slider2');
		correctNumber=0
		wrongNumber = 0;
	    ...
	    if(_.isEqual(check, answer)) {
			...
			if((this.state.values[(this.state.values.length)-1].Test1Level1CorrectNumber%3) == 1) {
				console.log("Answer is correct :-D");
				incompleteLevel = 2;
				if(this.state.values[(this.state.values.length)-1].Test1Level1CorrectNumber == null)
				{
					correctNumber = 1;
					wrongNumber = 0;
				} else {
					correctNumber=Number(this.state.values[(this.state.values.length)-1].Test1Level1CorrectNumber) +1;
					wrongNumber = 0;
				}
				Meteor.call('links.insert2',value1, value2, correctNumber, wrongNumber, incompleteLevel, Test1Attempts, Test1TotalCorrect, Test1TotalWrong);
			...
			if((this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber%3) == 1) {
				console.log("Answer is incorrect :-(");
				incompleteLevel = 0;
				if(this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber == null)
				{
					correctNumber = 0;
					wrongNumber = 1;
				} else {
					wrongNumber=Number(this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber) +1;
					correctNumber = 0;
				}
				Meteor.call('links.insert2',value1, value2, correctNumber, wrongNumber, incompleteLevel, Test1Attempts, Test1TotalCorrect, Test1TotalWrong);
				//this.setState({correctNumber: 0});
				setTimeout(() => history.push('/Test1Level0'), 0);
			} else {
				console.log("Answer is incorrect :-(");
				// this.setState({attemptNumber: this.state.attemptNumber + 1});
				if(this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber == null)
	...
	componentDidMount() {
	    ...
	    var x = document.getElementById('redSlider');
	    var y = document.getElementById('greenSlider');
 ...
 //  	clickZZZ = (event) => {
 //  		if((this.state.values[(this.state.values.length)-1].Test1Level1CorrectNumber) == null || (this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber) == null) {
	// 		var correctNumber = 0;
	// 		var wrongNumber = 0;
	// 		console.log("CorrectNumber: ",correctNumber);
	// 		console.log("WrongNumber  : ",wrongNumber);
	// 	} else {
	// 		console.log("CorrectNumber: ",this.state.values[(this.state.values.length)-1].Test1Level1CorrectNumber);
	// 		console.log("WrongNumber  : ",this.state.values[(this.state.values.length)-1].Test1Level1WrongNumber);
	// 	}
				...
				<div>
					...
					<form onSubmit={this.onSubmit.bind(this)} id="form">
						<div className = "slider1" id="redSlider">
							<input type="range" id="redSlider" ref="slider1" min="0" max="10" id="Level1Slider1" className="Level1Slider1" value={this.state.firstSlider, this.state.slidervals[0].value} onChange={this.handleFirstSlider.bind(this)}/>
						</div>
						<div className = "slider2" id="greenSlider">
							<input type="range" ref="slider2" min="0" max="10" id="Level1Slider2" className="Level1Slider2" value={this.state.secondSlider, this.state.slidervals[1].value} onChange={this.handleSecondSlider.bind(this)}/>
						</div>
