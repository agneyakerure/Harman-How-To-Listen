...
export default class Test1Level2 extends Component {
	constructor(props) {
		super(props);
		this.state = {
			firstSlider:0,
	...
	onSubmit(e) {
		...
		var z = document.getElementById('Level1Slider3');
		correctNumber=0
		wrongNumber = 0;
	    ...
	    if(_.isEqual(check, answer)) {
			...
			if(this.state.values[(this.state.values.length)-1].Test1Level2CorrectNumber >= 2) {
				...
				if(this.state.values[(this.state.values.length)-1].Test1Level2CorrectNumber == null)
				{
					...
					correctNumber=Number(this.state.values[(this.state.values.length)-1].Test1Level2CorrectNumber) +1;
					wrongNumber = 0;
				}
			...
			if(this.state.values[(this.state.values.length)-1].Test1Level2WrongNumber >= 2) {
				...
				if(this.state.values[(this.state.values.length)-1].Test1Level2WrongNumber == null)
				{
					...
					wrongNumber=Number(this.state.values[(this.state.values.length)-1].Test1Level2WrongNumber) +1;
					correctNumber = 0;
				}
				Meteor.call('links.insert3',value1, value2, value3, correctNumber, wrongNumber, incompleteLevel, Test1Attempts, Test1TotalCorrect, Test1TotalWrong);
	...
	componentDidMount() {
	    ...
	    var z = document.getElementById('blueSlider');
 ...
 //  	clickZZZ = (event) => {
 //  		if(this.state.values[(this.state.values.length)-1].Test1Level2CorrectNumber == null || this.state.values[(this.state.values.length)-1].Test1Level2WrongNumber == null) {
	// 		var correctNumber = 0;
	// 		var wrongNumber = 0;
	// 		console.log("CorrectNumber: ",correctNumber);
	// 		console.log("WrongNumber  : ",wrongNumber);
	// 	} else {
	// 		console.log("CorrectNumber: ",this.state.values[(this.state.values.length)-1].Test1Level2CorrectNumber);
	// 		console.log("WrongNumber  : ",this.state.values[(this.state.values.length)-1].Test1Level2WrongNumber);
	// 	}
				...
				<div>
					...
					<form onSubmit={this.onSubmit.bind(this)} id="form">
						<div className = "slider1" id="redSlider">
							<input type="range" ref="slider1" min="0" max="10" id="Level1Slider1" className="Level1Slider1" value={this.state.firstSlider, this.state.slidervals[0].value} onChange={this.handleFirstSlider.bind(this)}/>
						</div>
						...
						<div className = "slider3" id="blueSlider">
							<input type="range" ref="slider3" min="0" max="10" id="Level1Slider3" className="Level1Slider3" value={this.state.thirdSlider, this.state.slidervals[2].value} onChange={this.handleThirdSlider.bind(this)}/>
						</div>

						<button id="submit">Submit!</button>
					</form>
					{this.state.slidervals[0].value} - {this.state.slidervals[1].value} - {this.state.slidervals[2].value}
