...
import  Test4Level1Graph  from '../Test4/Test4Level1Graph.jsx';
import Modal from 'react-modal';
//For React Router
const history = createBrowserHistory({forceRefresh: true});

//Level Variables
var Test5Level1CorrectNumber = 0;
var Test5Level1WrongNumber = 0;
...
export default class Test5Level1 extends Component {
	constructor(props) {
		super(props);
		this.state = {
			isOpen: false,
	...
	onModalOk = (event) => {
		...
		if(this.state.test5[(this.state.test5.length)-1].Test5Level1CorrectNumber == null) {
			Test5Level1CorrectNumber = 0;
		} else {
			Test5Level1CorrectNumber = this.state.test5[(this.state.test5.length)-1].Test5Level1CorrectNumber;
		}
		if(this.state.test5[(this.state.test5.length)-1].Test5Level1WrongNumber == null) {
			Test5Level1WrongNumber = 0;
		} else {
			Test5Level1WrongNumber = this.state.test5[(this.state.test5.length)-1].Test5Level1WrongNumber;
		}
		Test5Attempts += 1;
		if(this.state.isCorrect == "Correct") {
			Test5TotalCorrect += 1;
			Test5Level1WrongNumber = 0;
			if(this.state.test5[(this.state.test5.length)-1].Test5Level1CorrectNumber == undefined) {
				Test5Level1CorrectNumber = 1;
			} else {
				console.log("here");
				Test5Level1CorrectNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level1CorrectNumber) +1;
			}

			if((Test5Level1CorrectNumber % 3) == 0) {
				Meteor.call('test5.Test5Level1Insert',Test5Level1CorrectNumber, Test5Level1WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => history.push('/Test5Level2'), 0); //go forward here
			} else {
				Meteor.call('test5.Test5Level1Insert',Test5Level1CorrectNumber, Test5Level1WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
			...
			Test5TotalWrong += 1;
			Test5Level1CorrectNumber = 0;
			if(this.state.test5[(this.state.test5.length)-1].Test5Level1WrongNumber == undefined) {
				Test5Level1WrongNumber = 1;
			} else {
				console.log("here");
				Test5Level1WrongNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level1WrongNumber) +1;
			}

			if((Test5Level1WrongNumber % 3) == 0) {
				Meteor.call('test5.Test5Level1Insert',Test5Level1CorrectNumber, Test5Level1WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
	...
	render() {
		var corno;
  		...
  		if(this.state.test5[(this.state.test5.length)-1]) {
  			corno = this.state.test5[(this.state.test5.length)-1].Test5Level1CorrectNumber;
  			wrongno = this.state.test5[(this.state.test5.length)-1].Test5Level1WrongNumber;
  		} else {
  			corno = 0;
  			wrongno = 0;
  		}
