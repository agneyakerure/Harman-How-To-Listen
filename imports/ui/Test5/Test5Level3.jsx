...
import  Test5Level3Graph  from '../Test4/Test4Level3Graph.jsx';
import Modal from 'react-modal';
//For React Router
const history = createBrowserHistory({forceRefresh: true});

//Level Variables
var Test5Level3CorrectNumber = 0;
var Test5Level3WrongNumber = 0;
...
export default class Test5Level3 extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	correct = (event) => {
  	...
  	onModalOk = (event) => {
		...
		if(this.state.test5[(this.state.test5.length)-1].Test5Level3CorrectNumber == null) {
			Test5Level3CorrectNumber = 0;
		} else {
			Test5Level3CorrectNumber = this.state.test5[(this.state.test5.length)-1].Test5Level3CorrectNumber;
		}
		if(this.state.test5[(this.state.test5.length)-1].Test5Level3WrongNumber == null) {
			Test5Level3WrongNumber = 0;
		} else {
			Test5Level3WrongNumber = this.state.test5[(this.state.test5.length)-1].Test5Level3WrongNumber;
		}
  		Test5Attempts += 1;
    	if(this.state.isCorrect == "Correct") {
    		Test5TotalCorrect += 1;
    		Test5Level3WrongNumber = 0;
    		if(this.state.test5[(this.state.test5.length)-1].Test5Level3CorrectNumber == undefined) {
				Test5Level3CorrectNumber = 1;
			} else {
				console.log("here");
				Test5Level3CorrectNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level3CorrectNumber) +1;
			}

			if((Test5Level3CorrectNumber % 3) == 0) {
				incompleteLevel = 4;
				Meteor.call('test5.Test5Level3Insert',Test5Level3CorrectNumber, Test5Level3WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => history.push('/Test5Level5'), 0); //go forward here
			} else {
				incompleteLevel = 3;
				Meteor.call('test5.Test5Level3Insert',Test5Level3CorrectNumber, Test5Level3WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
    		...
    		Test5TotalWrong += 1;
    		Test5Level3CorrectNumber = 0;
    		if(this.state.test5[(this.state.test5.length)-1].Test5Level3WrongNumber == undefined) {
				Test5Level3WrongNumber = 1;
			} else {
				console.log("here");
				Test5Level3WrongNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level3WrongNumber) +1;
			}

			if((Test5Level3WrongNumber % 3) == 0) {
				incompleteLevel = 2;
				Meteor.call('test5.Test5Level3Insert',Test5Level3CorrectNumber, Test5Level3WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => history.push('/Test5Level2'), 0); //go back here
			} else {
				incompleteLevel = 3;
				Meteor.call('test5.Test5Level3Insert',Test5Level3CorrectNumber, Test5Level3WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => window.location.reload(), 0);
  	...
  	render() {
  		...
  		if(this.state.test5[(this.state.test5.length)-1]) {
  			corno = this.state.test5[(this.state.test5.length)-1].Test5Level3CorrectNumber;
  			wrongno = this.state.test5[(this.state.test5.length)-1].Test5Level3WrongNumber;
  		} else {
  			corno = 0;
  			wrongno = 0;
  		}
				...
				<div>
					<div>
						<div className = "chartBox">
							<Test5Level3Graph array={this.state.graphArray}/>
						</div>
