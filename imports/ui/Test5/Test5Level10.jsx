...
import  Test5Level10Graph  from '../Test4/Test4Level10Graph.jsx';
import Modal from 'react-modal';
//For React Router
const history = createBrowserHistory({forceRefresh: true});

//Level Variables
var Test5Level10CorrectNumber = 0;
var Test5Level10WrongNumber = 0;
...
export default class Test5Level10 extends Component {
	constructor(props) {
		super(props);
		this.state = {
	...
	correct = (event) => {
  	...
  	filterJ = (event) => {
		//console.log(array[0].frequency);
	    if(isConnectedToFilter) {
	      filter.type = newArray[9].type;
	      filter.frequency.value = newArray[9].frequency;
	      filter.Q.value = newArray[9].q;
	      filter.gain.value = newArray[9].gain;
	      isConnectedToFilter = true;
	    }
	    else {
	      source.connect(filter);
  	...
  	filterK = (event) => {
		//console.log(array[0].frequency);
	    if(isConnectedToFilter) {
	      filter.type = newArray[10].type;
	      filter.frequency.value = newArray[10].frequency;
	      filter.Q.value = newArray[10].q;
	      filter.gain.value = newArray[10].gain;
	      isConnectedToFilter = true;
	    }
	    else {
	      source.connect(filter);
  	...
  	onModalOk = (event) => {
		...
		if(this.state.test5[(this.state.test5.length)-1].Test5Level10CorrectNumber == null) {
			Test5Level10CorrectNumber = 0;
		} else {
			Test5Level10CorrectNumber = this.state.test5[(this.state.test5.length)-1].Test5Level10CorrectNumber;
		}
		if(this.state.test5[(this.state.test5.length)-1].Test5Level10WrongNumber == null) {
			Test5Level10WrongNumber = 0;
		} else {
			Test5Level10WrongNumber = this.state.test5[(this.state.test5.length)-1].Test5Level10WrongNumber;
		}
  		Test5Attempts += 1;
    	if(this.state.isCorrect == "Correct") {
    		Test5TotalCorrect += 1;
    		Test5Level10WrongNumber = 0;
    		if(this.state.test5[(this.state.test5.length)-1].Test5Level10CorrectNumber == undefined) {
				Test5Level10CorrectNumber = 1;
			} else {
				console.log("here");
				Test5Level10CorrectNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level10CorrectNumber) +1;
			}

			if((Test5Level10CorrectNumber % 3) == 0) {
				incompleteLevel = 0;
				Meteor.call('test5.Test5Level10Insert',Test5Level10CorrectNumber, Test5Level10WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => history.push('/Test5Level0'), 0); //go forward here
			} else {
				incompleteLevel = 10;
				Meteor.call('test5.Test5Level10Insert',Test5Level10CorrectNumber, Test5Level10WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
    		...
    		Test5TotalWrong += 1;
    		Test5Level10CorrectNumber = 0;
    		if(this.state.test5[(this.state.test5.length)-1].Test5Level10WrongNumber == undefined) {
				Test5Level10WrongNumber = 1;
			} else {
				console.log("here");
				Test5Level10WrongNumber=(this.state.test5[(this.state.test5.length)-1].Test5Level10WrongNumber) +1;
			}

			if((Test5Level10WrongNumber % 3) == 0) {
				incompleteLevel = 9;
				Meteor.call('test5.Test5Level10Insert',Test5Level10CorrectNumber, Test5Level10WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => history.push('/Test5Level9'), 0); //go back here
			} else {
				incompleteLevel = 10;
				Meteor.call('test5.Test5Level10Insert',Test5Level10CorrectNumber, Test5Level10WrongNumber, incompleteLevel, Test5Attempts, Test5TotalCorrect, Test5TotalWrong);
				setTimeout(() => window.location.reload(), 0);
  	...
  	render() {
  		...
  		if(this.state.test5[(this.state.test5.length)-1]) {
  			corno = this.state.test5[(this.state.test5.length)-1].Test5Level10CorrectNumber;
  			wrongno = this.state.test5[(this.state.test5.length)-1].Test5Level10WrongNumber;
  		} else {
  			corno = 0;
  			wrongno = 0;
  		}
				...
				<div>
					<div>
						<div className = "chartBox">
							<Test5Level10Graph array={this.state.graphArray}/>
						</div>
